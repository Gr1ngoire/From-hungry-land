...
class AuthService {
    public async signIn(userData: UserSignInDto): Promise<UserSignInResponseDto> {
        const {data} = await axiosService.post<UserSignInResponseDto>(`${ApiRoutes.AUTH}${AuthRoutes.SIGN_IN}`, userData);
        return data;
    }

    public async signUp(userData: UserSignUpDto): Promise<UserSignInResponseDto> {
        const {data} = await axiosService.post<UserSignInResponseDto>(`${ApiRoutes.AUTH}${AuthRoutes.SIGN_UP}`, userData);
        return data;
    }
